...
exports.darLikeEmp = async (req, res) => {
    ...
    try {
        ...
        if (usuario.likes.includes(perfilAlvoId)) {
            ...
            usuario.matches.push(match); // Adiciona o match aos matches do usuário
            await usuario.save();

            perfilAlvo.matches.push(match); // Adiciona o match aos matches do perfil alvo (empresa)
...
exports.desfazerMatch = async (req, res) => {
    ...
    try {
        ...
        const usuario = await Dev.findById(match.dev);
        const perfilAlvo = await Empresa.findById(match.empresa);
